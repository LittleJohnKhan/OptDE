...
class OptDE(object):
    ...
    def __init__(self, args):
        ...
        self.D_list = [Discriminator(features=args.D_FEAT).cuda() for _ in range(self.args.num_class)]
        ...
        self.models = {"Encoder": self.Encoder, "Decoder": self.Decoder, "DI": self.DI_Disentangler, "DS": self.DS_Disentangler, "MS": self.MS_Disentangler, "DIC": self.DI_Classifier, "DSC": self.DS_Classifier, "VP":self.V_Predictor, "D_list": self.D_list}
        ...
        visual_class_list=self.args.visual_class_choices.split(',')
        for i in range(self.args.num_class):
            pt_path=os.path.join(args.ckpt_load, visual_class_list[i]+'.pt')
            print('load pretrained model : ', pt_path)
            checkpoint = torch.load(pt_path, map_location=self.args.device) 
            self.D_list[i].load_state_dict(checkpoint['D_state_dict'])

        # evaluation mode for Dropout and BatchNorm
        for model_name in ["Encoder", "Decoder", "DI", "DS", "MS", "DIC", "DSC", "VP"]:
            self.models[model_name].eval()
        for i in range(self.args.num_class):
            if self.D_list[i] is not None:
                self.D_list[i].eval()
    ...
    def finetune(self, class_idx: int, bool_gt=False, save_curve=False, ith=-1):
        # forward
        if bool_gt:
        ...
        for stage, iteration in enumerate(self.iterations):
            for i in range(iteration):
                ...
                ### compute losses
                ftr_loss = self.criterion(self.ftr_net_list[class_idx], x_map, self.partial)
                ...
                if save_curve: #?????
                    ...
                    if ith == -1:
                        basename = 'class'+str(class_idx)+'_'+str(self.pcd_id)
                    else:
                        basename = 'class'+str(class_idx)+'_'+str(self.pcd_id)+'_'+str(ith)
    ...
    def test_virtual_one_batch(self, class_idx: int, use_ema=False, ith=-1):
        loss_dict = {}
        count = 0
        stage = 0
        ...
        ### compute losses
        #ftr_loss = self.criterion(self.ftr_net_list[class_idx], x_map, self.target)
        ftr_loss = self.criterion(self.ftr_net_list[class_idx], x, self.gt)
    ...
    def test_real_one_batch(self, class_idx: int, bool_gt=False, use_ema=False, ith=-1):
        loss_dict = {}
        count = 0
        stage = 0
    ...
    def train_consistency_one_batch(self, class_idx: int, curr_step, cons_feature, return_generated=False, ith=-1):
        """
        return cs_loss
        """        
        loss_dict = {}
    ...
    def train_domain_one_batch(self, class_idx: int, curr_step, alpha, switch_idx_default=None, ith=-1):
        """
        input: 
    ...
    def train_virtual_one_batch(self, class_idx: int, curr_step, ith=-1, complete_train=False):
        """
    ...
    def train_real_one_batch(self, class_idx: int, curr_step, epoch, ith=-1):
        """
        input self.partial
        ...
        if epoch < 160 * self.args.num_class:
